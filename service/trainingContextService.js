...
class TrainingContextService {
   ...
   async getContextsAsTrainer(userId, options = {}) {
      ...
      // ✅ ИСПРАВЛЕНО: получаем контексты с уже подгруженной информацией о дружбе
      const contexts = await TrainingContextRepository.getAsTrainerWithFriend(userId, status);

      console.log(`✅ TrainingContextService.getContextsAsTrainer: найдено ${contexts.length} контекстов`);
      return contexts;
   }
   ...
   async getContextsAsTrainee(userId, options = {}) {
      ...
      // ✅ ИСПРАВЛЕНО: получаем контексты с уже подгруженной информацией о дружбе
      const contexts = await TrainingContextRepository.getAsTraineeWithFriend(userId, status);

      console.log(`✅ TrainingContextService.getContextsAsTrainee: найдено ${contexts.length} контекстов`);
      return contexts;
   }
   ...
   async getActiveContexts(userId) {
      ...
      const [asTrainer, asTrainee] = await Promise.all([
         TrainingContextRepository.getAsTrainerWithFriend(userId, 'active'),
         TrainingContextRepository.getAsTraineeWithFriend(userId, 'active')
      ]);
